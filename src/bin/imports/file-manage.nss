menu(where=sel.count>0 type='file|dir|drive|namespace|back' mode="multiple" title="File management" image=icon.copy_as_path)
{
	menu(separator="after" title="Copy path" image=icon.copy_path)
	{
		item(where=sel.count > 1 title='Copy (@sel.count) selected' cmd=command.copy(sel(false, "\n")))
	...
	item(mode="single" type="file" title="Change extension" image=icon.rename cmd=if(input("Change extension", "Enter extension"), 
		io.rename(sel.path, path.join(sel.dir, sel.file.title + "." + input.result))))
	
	menu(separator="after" image=icon.select_all title="Selection")
	{
		item(title="Select all" image=icon.select_all cmd=command.select_all)
		item(title="Invert selection" image=icon.invert_selection cmd=command.invert_selection)
		item(title="Deselect" image=icon.select_none cmd=command.select_none)
	}
	
	item(type='file|dir|back.dir|drive' title='Take ownership' image=icon.run_as_administrator admin
		cmd args='/K takeown /f "@sel.path" @if(sel.type==1,null,"/r /d y") && icacls "@sel.path" /grant *S-1-5-32-544:F @if(sel.type==1,"/c /l","/t /c /l /q")')
	separator
	menu(title="Show / Hide" image=icon.view)
	{
		item(title="System files" image=inherit cmd='@command.togglehidden')
		item(title="File extensions" image=inherit cmd='@command.toggleext')
	}
	...
	menu(mode="single" type='file' find='.dll|.ocx' separator="before" title='Register server' image=icon.settings)
	{
	...
	item(where=!wnd.is_desktop title="Folder options" image=icon.folder_options cmd=command.folder_options)
}
